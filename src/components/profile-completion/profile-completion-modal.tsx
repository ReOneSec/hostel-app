...
import { PrivacyConsentStep } from "@/components/profile-completion/privacy-consent-step";
...
import { ProfileFormData } from "@/app/(dashboard)/student/complete-profile/page";
...
export function ProfileCompletionModal() {
  ...
  const [currentStep, setCurrentStep] = useState(0); // 0 = Consent Step
  ...
  useEffect(() => {
    if (session?.user) {
      const isComplete = session.user.isProfileComplete;
      const isStudent = session.user.role === "STUDENT" || session.user.role === "MONTHLY_MANAGER";
      
      if (isStudent && isComplete === false) {
        setIsOpen(true);
        if (!session.user.privacyConsentAt && currentStep > 0) {
          setCurrentStep(0);
        } else if (session.user.privacyConsentAt && currentStep === 0) {
          setCurrentStep(1);
        }
      } else {
        setIsOpen(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}> 
      {/* prevent closing by passing empty function */}
      <DialogContent className="max-w-3xl p-0 overflow-hidden bg-gradient-to-br from-background via-background to-accent/20 border-border/50 shadow-2xl [&>button]:hidden h-[90vh] sm:h-[85vh] flex flex-col">
        {/* Top Bar - Only show if consent is given */}
        {currentStep > 0 && (
          <div className="border-b bg-card/80 backdrop-blur-sm sticky top-0 z-10 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8">
          ...
          <div className={currentStep > 0 ? "mt-8" : "h-full"}>
            {currentStep === 0 && (
              <PrivacyConsentStep onConsentGiven={() => setCurrentStep(1)} />
            )}
            {currentStep === 1 && (
              <PersonalInfoStep
                data={formData}
          ...
          {currentStep === 5 && selfieUploaded && (
            <div className="mt-8 flex justify-center pb-8">
              <Button
                ...
                className="px-8 shadow-lg shadow-primary/20 cursor-pointer w-full sm:w-auto"
              >
                {isSubmitting ? (
                  <>
