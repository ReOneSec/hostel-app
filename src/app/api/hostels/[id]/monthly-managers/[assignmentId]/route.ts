...
export async function DELETE(
  ...
  try {
    ...
    const assignment = await prisma.monthlyManagerSession.findUnique({
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Check if they are managing any OTHER months/hostels before downgrading
      const otherActiveAssignments = await tx.monthlyManagerSession.count({
        where: {
      ...
      if (otherActiveAssignments === 0 && assignment.user.role === "MONTHLY_MANAGER") {
    ...
    const otherActiveAssignmentsCheck = await prisma.monthlyManagerSession.count({
      where: {
        userId: assignment.userId,
        isActive: true
      }
    ...
    await createAuditLog({
      ...
      action: "MONTHLY_MANAGER_REMOVED",
      entity: "Hostel",
      entityId: hostelId,
      oldValues: { managerId: assignment.userId, month: assignment.month, year: assignment.year },
    ...
    console.error("[MonthlyManagers DELETE]", error);
